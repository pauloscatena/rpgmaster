...
import * as percepcao from '../../src/bot/commands/percepcao';
import type { LlmProvider } from '../../src/llm/provider';

describe('routeInteraction', () => {
  const pool = {} as Pool;
  const claudeClient = {} as Anthropic;
  const llmProvider = { runTurn: vi.fn() } as unknown as LlmProvider;
  ...
  it('despacha /criar-campanha para criarCampanha.execute', async () => {
    ...
    await routeInteraction(interaction, pool, claudeClient, llmProvider);
  ...
  it('despacha /percepcao para percepcao.execute com llmProvider', async () => {
    const spy = vi.spyOn(percepcao, 'execute').mockResolvedValue(undefined);
    const interaction = { isChatInputCommand: () => true, isModalSubmit: () => false, commandName: 'percepcao' } as any;
    await routeInteraction(interaction, pool, claudeClient, llmProvider);
    expect(spy).toHaveBeenCalledWith(interaction, pool, llmProvider);
  });
  ...
  it('ignora comandos desconhecidos sem lançar erro', async () => {
    ...
    await expect(routeInteraction(interaction, pool, claudeClient, llmProvider)).resolves.toBeUndefined();
  });
});
